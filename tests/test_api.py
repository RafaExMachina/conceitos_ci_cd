...
def test_predict_endpoint_sucesso():
    # Simulando o cliente chamando nossa API corretamente
    response = client.post(
        "/predict",
        json={"feature1": 3.0, "feature2": 4.0},
    )
...
def test_predict_endpoint_erro_validacao():
    ...
    response = client.post(
        "/predict",
        json={
            "feature1": "texto_invalido",
            "feature2": 4.0,
        },
    )

    # O FastAPI (Pydantic) deve bloquear e retornar erro 422
